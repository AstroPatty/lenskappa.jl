...
function GumbelFit(bins::Array{Float64, 1}, values::Array{Float64,1})
...
function GeVFit(bins::Array{Float64,1}, values::Array{Float64,1})
    ...
    function objective(parameters::Array{Float64,1})
        ...
        # normalize by sum and bin width
        squared_deviations = (values - gev).^2
        return sum(squared_deviations)        
    end
...
function fit(bins::Array{Float64,1}, values::Array{<:Real,1}, type::Symbol = :GEV)
    ...
    if type == :Gumbel
        return GumbelFit(bins, values)
    elseif type == :GEV
        return GeVFit(bins, values)
    else
...
function fit(kappas::Array{Float64, 1}, n_bins::Int64, type::Symbol = :GEV)
